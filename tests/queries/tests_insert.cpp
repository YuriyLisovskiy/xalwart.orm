...
struct TestCase_Q_insert_TestModel : public orm::Model
{
	...
	{
		this->set_attribute_for<TestCase_Q_insert_TestModel>(TestCase_Q_insert_TestModel::meta_columns, attr_name, data);
	}

	[[nodiscard]]
	inline std::shared_ptr<const Object> __get_attr__(const char* attr_name) const override
	{
		return this->get_attribute_from<TestCase_Q_insert_TestModel>(TestCase_Q_insert_TestModel::meta_columns, attr_name);
	}
};
