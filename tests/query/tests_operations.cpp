...
#include "../../src/query/operations.h"
...
TEST(TestCase_Q_ordering, asc)
{
	std::string expected = "id ASC";
	auto actual = orm::q::asc("id");
	ASSERT_EQ(expected, (std::string)actual);
}

TEST(TestCase_Q_ordering, desc)
{
	std::string expected = "id DESC";
	auto actual = orm::q::desc("id");
	ASSERT_EQ(expected, (std::string)actual);
}
...
{
	std::string expected = "(id = 1 AND name = 'John')";
...
{
	std::string expected = "(id = 1 OR name = 'John')";
	auto actual = orm::q::condition("id = 1") | orm::q::condition("name = 'John'");
	ASSERT_EQ(expected, (std::string)actual);
}
