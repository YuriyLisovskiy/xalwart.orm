...
inline ColumnCondition between(
...
struct Join
{
	...
	Join(
		std::string type, std::string table_name, q::Condition condition
...
inline Join join_on(
	const std::string& type, const std::string& fk_to_left="", const q::Condition& extra_condition={}
)
{
